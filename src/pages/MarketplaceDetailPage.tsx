...
import {
  ...
  ChevronRight, Zap, UserPlus, Filter, Download,
...
export function MarketplaceDetailPage() {
  ...
  const otherByCreator = MARKETPLACE_SCREENERS
    .filter((s) => s.creator.handle === screener.creator.handle && s.id !== id)
    .slice(0, 2);
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-foreground mb-1">{screener.name}</h1>
        {screener.description && (
          <p className="text-sm text-muted-foreground max-w-2xl">{screener.description}</p>
        )}
      </div>
      ...
      <Card className="mb-6">
        <CardContent className="p-5">
          ...
          <code className="text-sm text-foreground bg-muted/70 p-4 rounded-lg block leading-relaxed whitespace-pre-wrap font-mono">
            {screener.conditions.split(" AND ").map((cond, i, arr) => (
              <span key={i}>
                <span className="text-primary font-medium">{cond.trim()}</span>
                {i < arr.length - 1 && (
                  <span className="text-muted-foreground"> AND{"\n"}</span>
                )}
              </span>
            ))}
          </code>
      ...
      <div className="flex items-center gap-2 mb-6 flex-wrap">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full h-9 pl-9 pr-3 text-sm rounded-md border border-border bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
          />
        </div>
        ...
        <Button
          ...
          className={cn("text-xs h-9 gap-1.5", liked && "bg-red-500 hover:bg-red-600 border-red-500")}
          onClick={() => setLiked(!liked)}
        >
          <Heart className={cn("w-3.5 h-3.5", liked && "fill-white")} />
          {liked ? "Liked" : "Like"} ({formatCount(screener.likes + (liked ? 1 : 0))})
        </Button>

        {/* Edit / Save this screener */}
        <Button asChild size="sm" variant="outline" className="text-xs h-9 gap-1.5 border-primary/30 text-primary hover:bg-primary/5">
          <Link to="/diy">
                  ...
                  {results.map((row) => (
                    <tr key={row.symbol} className="hover:bg-muted/30 transition-colors group">
                      ...
                      <td className="px-4 py-3">
                        ...
                        {/* B | S hover buttons */}
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 mt-1 ml-6">
            ...
            <CardContent className="p-4">
              ...
              {otherByCreator.length > 0 && (
                <div className="mt-3 pt-3 border-t border-border/60">
                  <p className="text-xs text-muted-foreground mb-2">More by this publisher</p>
            ...
            <CardContent className="p-4">
              ...
              {screener.performance.badge === "new" ? (
                <p className="text-xs text-muted-foreground">
                  Not enough data yet ({screener.performance.sampleSize} matches)
                </p>
              ) : (
                <div
                  className={cn(
                    "rounded-lg border p-3",
                    screener.performance.badge === "high"
                      ? "text-green-700 bg-green-50 border-green-200"
                      : "text-yellow-700 bg-yellow-50 border-yellow-200"
                  )}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-2xl font-bold">{screener.performance.hitRate}%</span>
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {screener.tags.map((t) => (
                  <span key={t} className="text-xs px-2 py-0.5 bg-muted rounded text-muted-foreground">
                    {t}
                  </span>
                ))}
              </div>
